...
public class GiftPoint {
     ...
     */
    public boolean isEnough(final ConsumptionPoint point) {
        return point.getValue() <= value;
    }
     ...
     * @return 소비 후 남은 포인트
     */
    public GiftPoint consume(final ConsumptionPoint point) {
        if (!isEnough(point)) {
            throw new IllegalArgumentException("포인트가 부족합니다.");
        }

        return new GiftPoint(value - point.getValue());
    }
